package com.xyf.demo.controller;

import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.xyf.demo.bean.*;
...
public class TeacherController {
    ...
    public String toteadmin(@PathVariable("pn") Integer pn, Model model) {
        ...
        List<Results> results = resultsService.getAllResult();
        PageInfo<Results> page = new PageInfo<Results>(results, 5);
    ...
    public String resAdd(@Valid Results results, BindingResult bindingResult, Model model) {
        List<ObjectError> allErrors = bindingResult.getAllErrors();
        List<MyError> errmsg = new ArrayList<>();
        Results resultsVail = null;
        if (allErrors.size() == 0) {
            if (studentService.selectById(results.getStuId()) != null) {
                resultsVail = resultsService.selectResultByStuIdAndSubName(results.getStuId(), results.getSubName());
                if (resultsVail == null) {
    ...
    public String updateResPage(@PathVariable("resId") int resId, Model model) {
        Results results = resultsService.selectResultByResId(resId);
        model.addAttribute("res", results);
    ...
    public String updateRes(@Valid Results results, BindingResult bindingResult, Model model) {
        ...
        if (allErrors.size() == 0) {
            ...
            resultsService.deleteResultById(results.getResId());
            resultsService.addResult(results);
            return "redirect:/tea/toteadmin/1";
        } else {
            for (ObjectError error : allErrors) {
    ...
    public String deleres(@PathVariable("resId") int stuId) {
        resultsService.deleteResultById(stuId);
        return "redirect:/tea/toteadmin/1";
    }
    ...
    public String selectResultByStuId(@PathVariable("pn") Integer pn, @Param("stuId") String stuId, Model model) {
        PageHelper.startPage(pn, 6);
        List<Results> results = resultsService.selectByStuId(stuId);
        PageInfo<Results> page = new PageInfo<Results>(results, 5);
        List<Classes> classes = classService.getAllClass();
        model.addAttribute("classes", classes);
        model.addAttribute("pageInfo", page);
    ...
    public String updateResById(@Valid Results results, BindingResult bindingResult, Model model) {
        ...
        if (allErrors.size() == 0) {
            ...
            resultsService.deleteResultById(results.getResId());
            resultsService.addResult(results);
            return "redirect:/tea/selectById/1?stuId=" + results.getStuId();
        } else {
            for (ObjectError error : allErrors) {
                errmsg.add(new MyError(error.getDefaultMessage()));
            }
    ...
    public String deleresById(@PathVariable("resId") int resId) {
        Results results = resultsService.selectResultByResId(resId);
        resultsService.deleteResultById(resId);
        return "redirect:/tea/selectById/1?stuId=" + results.getStuId();
    }
    ...
    public String selectRank(@Param("resTerm") String resTerm, Model model) {
        List<Rank> ranks = resultsService.selectRankByTerm(resTerm);
    ...
    public String selectRankbyclass(@Param("resTerm") String resTerm, @Param("className") String className, Model model) {
        ...
        if (resTerm.equals("") || className.equals("")) {
            ...
            List<Rank> ranks = resultsService.selectRankByTermAndStuClass(resTerm, className);
            model.addAttribute("ranks", ranks);
            model.addAttribute("resTerm", resTerm);
