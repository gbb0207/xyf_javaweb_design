...
public class StudentController {
    ...
    public String toresdmin(@PathVariable("pn") Integer pn, Model model, HttpSession httpSession) {
        ...
        List<Results> results = resultsService.selectByStuId((String) httpSession.getAttribute("loginUser"));
    ...
    public String selectResByTerm(@PathVariable("pn") Integer pn, @Param("resTerm") String resTerm, Model model, HttpSession httpSession) {
        PageHelper.startPage(pn, 9);
        List<Results> results = resultsService.selectByStuIdAndResTerm((String) httpSession.getAttribute("loginUser"), resTerm);
        PageInfo<Results> page = new PageInfo<Results>(results, 5);
        model.addAttribute("pageInfo", page);
